...
describe('target console', function () {
    ...
    it('should print a log to the console', async function () {
        ...
        this.logs.should.deep.eq([
            [`${time.format('DD.MM.YYYY HH:mm:ss.SSS')} [level][test component][test instance]: test title`],
            [`  ] {\n  ]   \"test\": "data"\n  ] }`]
        ]);
    });
    ...
    it('should use the timestamp config', async function () {
        ...
        this.logs.should.deep.eq([
            [`${time.format('HH-mm-ss-SSS')} [level][test component][test instance]: test title`]
        ]);
        await instnace.stop();
    });
